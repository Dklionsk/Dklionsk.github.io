/*
  Special thanks to Ben Wolf for providing the terminal interface.
*/
/*
...
var Terminal=(function(){
  ...
  function Stdin(element){
    ...
    this.enter=function(){
      if (!this.element.value.match(/^[ ]*exit\b/)) {
  ...
  var term={
	...
	if (term.pswdenter) {
		if (e.type=="keypress" && key==13) {
			...
			term.pswdeval(term.pswd);
	  		term.buf=[];
			term.prompt = term.promptI;
			term.pswdenter = false;
			term.pswd = '';
			term.stdin.enter();
			term.stdout.flush();
		...
		if (e.type=="keypress" && key==8) {
			if (term.pswd.length > 0) {
				term.pswd = term.pswd.slice(0, term.pswd.length-1);
			}
			cancelKey(e);
			scrollBottom();
			return false;
